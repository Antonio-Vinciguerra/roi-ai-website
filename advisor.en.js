(async () => {
 ...
 function addMessage(role,text) {
  ...
  const speaker=document.createElement('span');speaker.className='speaker';speaker.textContent=role==='user'?'You':'Heading South';
  article.append(speaker,document.createTextNode(text));log.append(article);log.scrollTop=log.scrollHeight;
 }
 addMessage('assistant',live ? 'Hello. What would you like to make clearer about your business or technology plans?' : 'Hello—welcome to Heading South. You can explore our approach and where we could help your business. This preview uses guided answers; the live advisor comes next.');
 function stopVoice() {
