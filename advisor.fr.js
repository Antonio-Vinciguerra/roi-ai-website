(async () => {
 ...
 function addMessage(role,text) {
  ...
  const speaker=document.createElement('span');speaker.className='speaker';speaker.textContent=role==='user'?"Vous":'Heading South';
  article.append(speaker,document.createTextNode(text));log.append(article);log.scrollTop=log.scrollHeight;
 }
 addMessage('assistant',live ? "Bonjour. Que souhaitez-vous clarifier concernant votre entreprise ou vos projets technologiques ?" : "Bonjour, bienvenue chez Heading South. Découvrez notre approche et comment nous pourrions aider votre entreprise. Cet aperçu utilise des réponses guidées ; le conseiller IA viendra ensuite.");
 function stopVoice() {
